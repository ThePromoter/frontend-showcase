          ...
          //>>excludeStart('excludeHbs', pragmas.excludeHbs)
            ...
            function fetchAndRegister(langMap){
              fetchText(path, function (text) {
                  ...
                  if(config.hbs.debug && !config.isBuild) {
                      console.groupCollapsed('Loaded a template: ' + name);
                      
                      console.groupCollapsed('MetaData' + (metaObj ? '' : ': NONE'));
                      if(metaObj) { console.log(metaObj); }
                      console.groupEnd();
                      
                      console.groupCollapsed('Variables: ' + vars.length);
                      if(vars.length > 0) { console.log(vars); }
                      console.groupEnd();
                      
                      console.groupCollapsed('Dependencies: ' + deps.length);
                      if(deps.length > 0) { console.log(deps); }
                      console.groupEnd();
                      
                      console.groupCollapsed('Helpers: ' + helps.length);
                      if(helps.length > 0) { console.log(helps); }
                      console.groupEnd();
